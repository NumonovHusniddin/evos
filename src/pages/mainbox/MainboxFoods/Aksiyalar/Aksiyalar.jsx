...
function Aksiyalar() {
  const { data: aksiya, isLoading } = UseFetch("https://picdnlswhqdzagojulhf.supabase.co/storage/v1/object/sign/EVOS%20json/db/db.json?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJFVk9TIGpzb24vZGIvZGIuanNvbiIsImlhdCI6MTc0MjU0OTA2MiwiZXhwIjoxNzc0MDg1MDYyfQ.vfpybu2AHKzJbovRgnD81v9jPC4TnvQUpogmDQGtB7w");
  const { t, i18n } = useTranslation();
  return (
    <div>
      {isLoading && <h1>Loading...</h1>}
      <div className="grid grid-cols-4 gap-x-2 gap-y-3">
        {aksiya?.foods?.aksiyalar &&
          aksiya?.foods?.aksiyalar.map((item, id) => (
            <div
              key={id}
            ...
            >
              ...
              <Link className="w-full" to={`/aksiya/${item.urlAksiya}`}>
