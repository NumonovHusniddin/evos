...
function HotDogLocation() {
  const param = useParams()
  const paramHotDog = param.hotdog
  const { data: hotdog, isLoading } = UseFetch("https://ixhhcbgznqctfsbitmtv.supabase.co/storage/v1/object/sign/api/db.json?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV85ODVjMzJmOS02MjdjLTQwNjMtODIwYi1hZDMwMGI0MzRlMGQiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJhcGkvZGIuanNvbiIsImlhdCI6MTc1OTU3MzgxNywiZXhwIjoxNzkxMTA5ODE3fQ.2ZNDa3HNjisRb1Knp3qVBmoAQ0n2cUGN_rj1JswXRmQ")
  const findHotDog = hotdog?.foods?.hotdog.find((item) => item.urlHotdog === paramHotDog)
  const { t, i18n } = useTranslation()
  const navigate = useNavigate()

  const Back = () => {
    navigate("/menu/hot-dog")
  }
  return (
