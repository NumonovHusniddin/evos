...
function FindRamazan() {
  const [data, setData] = useState(null);
  const [isloading, setISLoading] = useState(false);
  const param = useParams();
  const paramBreak = param.ramazan;
  const findRamazan = data?.foods?.ramazon.find(
    (item) => item.urlRamazon === paramBreak
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      setISLoading(true);
      const request = await fetch("https://ixhhcbgznqctfsbitmtv.supabase.co/storage/v1/object/sign/api/db.json?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV85ODVjMzJmOS02MjdjLTQwNjMtODIwYi1hZDMwMGI0MzRlMGQiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJhcGkvZGIuanNvbiIsImlhdCI6MTc1OTU3MzgxNywiZXhwIjoxNzkxMTA5ODE3fQ.2ZNDa3HNjisRb1Knp3qVBmoAQ0n2cUGN_rj1JswXRmQ");
