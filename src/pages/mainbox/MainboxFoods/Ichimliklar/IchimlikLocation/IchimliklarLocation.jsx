...
function IchimliklarLocation() {
  const param = useParams();
  const paramIchimlik = param.ichimlik;
  const { data: ichimlik, isLoading } = UseFetch("https://picdnlswhqdzagojulhf.supabase.co/storage/v1/object/sign/EVOS%20json/db/db.json?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJFVk9TIGpzb24vZGIvZGIuanNvbiIsImlhdCI6MTc0MjU0OTA2MiwiZXhwIjoxNzc0MDg1MDYyfQ.vfpybu2AHKzJbovRgnD81v9jPC4TnvQUpogmDQGtB7w");
  const findHotDog = ichimlik?.foods?.ichimliklar.find(
    (item) => item.urlIchimlik === paramIchimlik
  );
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();

  const Back = () => {
    navigate("/menu/ichimliklar");
  };
  return (
    <div className=" container pt-[170px]">
      {isLoading && <h1>Loading...</h1>}
      <div className="flex flex-col items-center justify-center">
        <img className="max-w-[600px]" src={findHotDog?.image} alt="" />
        <div className="flex flex-col items-center">
          <h1 className="text-black dark:text-white text-[40px] text-center mb-2">
            {findHotDog?.title[i18n.language]}
          </h1>
          <p className="text-black dark:text-white text-center max-w-[800px] m-auto font-semibold mb-4 text-[25px]">
            {findHotDog?.desc[i18n.language]}
          </p>

          <p className="text-center font-bold text-[30px] dark:text-white mb-3">
            {t("narx")}: {""}
            <span className=" text-green-600">{findHotDog?.price}</span>
          </p>
...
export default IchimliklarLocation;
