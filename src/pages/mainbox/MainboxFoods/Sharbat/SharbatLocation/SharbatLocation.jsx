...
function SharbatLocation() {
  ...
  const paramSharbat = param.sharbatlar
  const findSharbat = loc?.foods?.sharbat.find((item) => item?.urlSharbat === paramSharbat)
  const { t, i18n } = useTranslation()
  const navigate = useNavigate()

  const Back = () => {
    navigate("/menu/sharbatlar")
  }
  return (
    <div className="pt-[130px] container">
      {isLoading && <h1>Loading...</h1>}
      <div className="flex flex-col items-center justify-center">
        <img className="max-w-[600px]" src={findSharbat?.image} alt="" />
        <div className="flex flex-col items-center">
          <h1 className="text-black dark:text-white text-[40px] text-center mb-2">
            {findSharbat?.title[i18n.language]}
          </h1>
          <p className="text-black dark:text-white text-center max-w-[800px] m-auto font-semibold mb-4 text-[25px]">
            {findSharbat?.desc[i18n.language]}
          </p>

          <p className="text-center font-bold text-[30px] dark:text-white mb-3">
            {t("narx")}: {""}
            <span className="text-green-600 ">{findSharbat?.price}</span>
          </p>
...
export default SharbatLocation
